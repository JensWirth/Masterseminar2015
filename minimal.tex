...
\end{equation}
Dies zeigt $\spann\left(W(\mcP_a) W(\mcP_b)\right)\subs W(\mcP)$. 
...
\frac{\til{Q}(\x)}{\til{P}(\x)}\rightarrow0 \quad\text{für}\quad\x\rightarrow\infi.
\end{equation}
\end{thm}
...
\begin{equation}
\frac{\til{Q}(\x)^2}{\til{P}(\x)^2} = \frac{\sum_{|\alpha| \leq k} | Q^{(\alpha)} (\x)|^2  }{ \sum_{|\alpha| \leq k} | P^{(\alpha)} (\x)|^2} \rightarrow 0\ \ \text{für} \ \ \x \rightarrow \infty
\end{equation}
...
\begin{equation}
|Q(\x)|^2 \leq C \sum_{|\alpha| \leq k} | P^{\alpha} (\x)|^2 \label{eq:1.5}
\end{equation}
...
\end{equation}
ergibt. Damit können wir zeigen, dass die Funktionen $\widehat{Q(\D)}u_n = Q(\x) \hat{u}_n$ gleichgradig stetig und gleichmäßig beschränkt sind. Sei dazu $\varepsilon>0$, dann existiert ein $\delta>0$, so dass aus $|\xi_1 - \xi_2| < \delta$ und der Ungleichung von Cauchy-Schwarz 
\begin{align}
...
\end{align}
folgt. Also können wir nach dem Satz von Arzel\`a-Ascoli eine lokal gleichmäßig konvergente Teilfolge $Q(\x)\widehat{u}_{n_k}$ auswählen, d.h. $Q(\x)\widehat{u}_{n_k}$ konvergiert auf jeder kompakten Menge $K \subseteq \Omega$ gleichmäßig.
...
\end{equation}
was die Konvergenz der Teilfolge $\left(Q(\D)u_{n_k} \right)_{k \in \mathbb{N}}$ impliziert. Da die Funktionen $P(\D)u$ für $u\in C^\infi_0(\Om)$ dicht in $\mcR_{P_0}$ liegen, dominiert $P(\D)$ den Operator $Q(\D)$.
...
\begin{equation}
P(\D)u_n(x)=\e^{\i x \cdot \x_n}\sum_\al\frac{P^{(\al)}(\x_n)}{\til{P}(\x_n)}\frac{D^\al u(x)}{\al!}.
\end{equation}
...
\begin{df}
Sei $P(\x)$ das Symbol eines Differentialausdrucks $P(\D)$. Wir sagen dass $P(\x)$ vollständig ist, wenn folgendes gilt:
\begin{align*}
...
\\ \textit{FEHLT} %Wir werden hierbei induktiv vorgehen. Für $\deg(P)=m=1$ ist die Beschränktheit klar. Wir nehmen also an, dass die Behauptung für Polynome vom Grad kleiner als $m$ gilt. Existiert ein Vektor $\xi_0$ mit $P^{\alpha}_m(\xi_0) = 0, \ \forall \alpha \in \mathbb{N}_0^n, |\alpha| = m-1$, dann gilt nach Lemma \ref{Lema:homogene Polynome} bereits $\xi_0 \in \Lambda(P_m)$. Damit ist klar, dass $\tilde{P}_m (\xi)$ in $M_C$ beschränkt ist
\end{proof}

Wir wollen zum Abschluss des Kapitels ein weiteres Vergleichsresultat angeben welches ebenfalls aus dem Quotienten der regularisierten Symbole $\tilde{P}$ und $ \tilde{Q}$ hervorgeht. Es handelt sich hierbei um die Fortsetzung des Definitionsbereiches von $Q(\D)$ auf den des minimalen Operators $P_0^{-1}$.
\begin{thm}
