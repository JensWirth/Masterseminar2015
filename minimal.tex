...
%%
%%Mit der Leibnizformel als Motivation,
%%wählen wir als Ansatz für die Regularisierungen
%\begin{equation}\label{eq:2:2}
...
%$\til{P}(\x)$ ein reelles Polynom mit $\Grad\til{P}(\x)=2\Grad P(\x)$ ist,
%das keine reellen Nullstellen besitzt.
%Der Beweis des folgenden zentralen Satzes in diesem Kapitel zeigt, 
%dass \eqref{eq:2:2} in der Tat eine geeignete Regularisierung ist.
...
\end{equation}
Die Ungleichungen \eqref{eq:2.7} und \eqref{eq:2.8} kombinieren für $t_\alpha=Q^{(\alpha)}(\xi)$ und damit $\widetilde Q(\xi)^2=|t|^2$ zu
\begin{equation}\label{eq:2.9}
\til{Q}(\x)^2 \Psi_- \le C  \bigg(\sum_{\al,\be}P^{(\al)}(\x)\overline{{P}^{(\be)}(\x)}\Psi_{\al\be}+\Psi_{00}\bigg) \leq 2 C \til{P}(\x)^2 \Psi_+
\end{equation}
...
Unser Ziel ist es, Ableitungen von Differentialpolynomen zu kontrollieren.
