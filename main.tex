\documentclass[11pt,bibtotoc]{scrbook}
\usepackage{german}
\usepackage[utf8]{inputenc}
%
\usepackage{hyperref}
%
\usepackage{wasysym}\let\iint\relax\let\iiint\relax
\usepackage{amsfonts,amssymb,amsthm,amsmath,CJK,mathrsfs}
\usepackage{braket}
\usepackage[a4paper, scale=0.76]{geometry}
%
\usepackage{ifthen}
\usepackage{makeidx}
\makeindex
\newcommand{\eIndex}[2][]{%
\emph{#2}%
\ifthenelse{\equal{#1}{}}{\index{#2}}{\index{#1!#2}}%	
}%
\def\spro#1#2{\pmb(#1,#2\pmb)}
\def\cspro#1#2#3{\pmb\langle#1\pmb|#2\pmb|#3\pmb\rangle}
\def\abs#1{\lvert#1\rvert}
\def\Abs#1{\left\lvert#1\right\rvert}
\def\norm#1{\lVert#1\rVert}
\def\Norm#1{\left\lVert#1\right\rVert}
%
% Aufzaehlungen roemisch numeriert....
\renewcommand{\labelenumi}{{\rm\bf(\roman{enumi})}}
%
\newtheorem{thm}{Satz}[chapter]
\newtheorem{lem}[thm]{Lemma}
\newtheorem{cor}[thm]{Korollar}
\renewcommand{\proofname}{Beweis}
\theoremstyle{definition}
\newtheorem{df}[thm]{Definition}
\newtheorem{exa}[thm]{Beispiel}
\newtheorem{rem}[thm]{Bemerkung}
%
%
% DEFINITIONEN
\def\N{\mathbb N}    % N
\def\R{\mathbb R}    % Koerper R
\def\C{\mathbb C}    % Koerper C
\def\Z{\mathbb Z}
\def\e{\mathrm e}     % Eulersche Zahl e
\def\i{\mathrm i}       % imaginäre Einheit i
\def\D{\mathrm D}    % Differentialoperator D
\def\d{\,\mathrm d}   % Differential d für Integrale
\def\rmC{\mathrm C}% Raum C
\def\rmL{\mathrm L} % Raum L
\def\rmH{\mathrm H}% Raum H
\def\rmB{\mathrm B}
\def\mcP{\mathcal P}   % curly P
\def\mcQ{\mathcal Q}  % curly Q  
\def\mcR{\mathcal R}  % curly R
\let\epsilon\varepsilon
%% von Tillmann
% NOT TO BE USED, WILL BE REMOVED LATER
%\def\pr{\prime}
%\def\tr{\top}%{\mathrm{T}}
%\def\al{\alpha}
%\def\be{\beta}
%\def\de{\delta}
%\def\ep{\epsilon}
%\def\ga{\gamma}
%\def\x{\xi}
%\def\y{\eta}
%\def\z{\zeta}
%\def\Om{\Omega}
%\def\pd{\partial}
%\def\infi{\infty}
%\def\til#1{\widetilde{#1}}
%\def\mto{\mapsto}
%\def\ti{\times}
%\def\Lap{\Delta}
\def\cc#1{\overline{#1}}
\def\ul#1{\underline{#1}}
\def\ska#1#2{\langle#1,#2\rangle}
\def\Ska#1#2{\left\langle#1,#2\right\rangle}
%\def\subs{\subset}
%\def\Lin{\operatorname{span}}
%\def\Grad{\operatorname{deg}}
\def\gdw{\Leftrightarrow}
%%%%%%%%%%%%%%%%%%%%
%
\DeclareMathOperator{\supp}{supp}          % Traeger (supp)
\DeclareMathOperator{\diam}{diam}          % Durchmesser  (diam)
\DeclareMathOperator{\dist}{dist}               % Abstand (dist)
\let\div\relax
\DeclareMathOperator{\div}{div}                % Divergenz
\DeclareMathOperator{\spann}{span}        % Lineare Huelle (span)
\let\Re\relax
\DeclareMathOperator{\Re}{Re}
\let\Im\relax
\DeclareMathOperator{\Im}{Im}
%
\begin{document}
\titlehead{Priv.-Doz. Dr. Jens Wirth\\Institut f\"ur Analysis, Dynamik und Modellierung\\Universit\"at Stuttgart}
\lowertitleback{\copyright 2015. Texte geschrieben von den Teilnehmern des Seminars.  Einzelne Beiträge sind nicht namentlich gekennzeichnet. Ausarbeitungen basieren auf Originalliteratur. \\
}
\title{Mikrolokale Analysis}
\subtitle{Masterseminar}
\author{---}
\date{Sommersemester 2015}
\maketitle
%
\section*{Vorwort}
Die hier zusammengetragenen Texte entstanden im Rahmen des Masterseminars
\begin{quote}\em 
Mikrolokale Analysis
\end{quote}
im Sommersemester 2015. Sie basieren auf Originalliteratur und zeichnen damit insbesondere auch die Entwicklung zugrundeliegender Ideen nach. Schwerpunkt waren Ungleichungen für Operatoren und quadratische Formen, ausgehend von Hörmanders fundamentaler Arbeit zu Differentialoperatoren mit konstanten Koeffizienten werden Vergleichsresultate diskutiert. Im Anschluß daran diskutieren wir lokale Lösbarkeit von Operatoren mit variablen Koeffizienten und das Beispiel von Lewy sowie Ungleichungen für quadratische Formen nach G\r{a}rding und Melin.

\bigskip
\noindent
Am Seminar beteiligt waren als Studenten
\begin{itemize}
\item Simon Barth,
\item Andreas Bitter,
\item Jonas Brinker,
\item Thomas Hamm,
\item Matthias Hofmann,
\item Tillmann Kleiner,
\item Robin Lang, sowie
\item Julian Mauersberger,
\end{itemize}
denen für die Zusammenstellung dieses Textes hiermit gedankt sei.

\vspace{2cm}

{\raggedleft Jens Wirth\\
Stuttgart, im Juli 2015 \\
}

\tableofcontents
%
\part{Analysis von Differentialoperatoren}
%
\include{einleitung1} % Einleitung, Teil 1
\include{minimal} % Minimale Operatoren :: Tilmann Kleiner, Andreas Bitter
\include{maximal} % Maximale Operatoren :: Thomas Hamm, Matthias Hofmann
\include{variabel} % Operatoren von reellem Haupttyp :: Julian Mauersberger
%
%
\part{Untere Schranken an Pseudodifferentialoperatoren}
%
\include{einleitung2} % Einleitung, Teil 2
\include{garding}  % Gardingsche Ungleichung :: Simon Barth
\include{melin} % Ungleichung von Melin :: Jonas Brinker

%
%

% Literaturverzeichnis
\bibliographystyle{alpha}
\bibliography{seminar}
% Index
\printindex
\end{document}
