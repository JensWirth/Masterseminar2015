...
Damit kommen wir zum Hauptresultat dieses Abschnitts, der schwarfen G\r{a}rdingungleichung für Systeme nach Lax und Nirenberg.
...
\begin{equation}
\begin{split}	
| I| &\le \frac{C}{2}\iint| \cspro{\widehat{u}(\xi)}{\widehat{a}(\xi-\eta,\eta)}{\widehat{u}(\eta)}| \, |\xi-\eta|^2\langle \xi\rangle^{-\frac{1}{2}} \langle\eta\rangle^{-\frac{1}{2}}\d \xi \d \eta\\ 
& \le \frac{1}{2}| a|_{0,2}|| u||_{(-{1}/{2})}^2.
\end{split}
\end{equation}
...
\end{equation}
zu zeigen.\\
Für jedes $l\in\{1,\cdots n\}$ gilt für $\xi = \pm c\langle\tau\rangle^{\frac{1}{2}}\e_l$ mit einem später geschickt gewählten $c$, dass
\begin{equation}
...
\end{equation}
positiv definit. Weil Addieren eines Terms der Form $c \langle\tau\rangle^{-1}$ mit \eqref{Abschaetzung 0-Norm} einen Fehler der Ordnung $O\left(\vert\vert v\vert\vert_{\left(-\frac{1}{2}\right)}^2\right)$ verursacht, können wir ohne Einschränkung können wir annehmen, dass  
\begin{equation}
