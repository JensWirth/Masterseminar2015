...
Sei $P(\D)$ eine Differentialausdruck und $\Omega\subset\R^n$ ein Gebiet. Wir erinnern an die Definition des maximalen Operators $P$, dieser versteht als größtmögliche distributionelle Fortsetzung von $P(D)$ und ist auf dem Definitionsbereich
\begin{equation}
...
\begin{equation}
P(\D)v=\sum_\alpha \dfrac{\D^\alpha \varphi}{\abs{\alpha}!}P^{(\alpha)}(\D)u
\end{equation}
...
\begin{equation}
\widetilde{P}(\zeta)=\Big(\sum_\alpha \abs{P^{(\alpha)}(\zeta)}^2\Big)^{1/2} \leq C(1+\abs{P(\zeta)}^2)
\end{equation}
