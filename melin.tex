...
\begin{equation}
   P[u] = \Re\int \overline{u(x)} \sum_{|\alpha+\beta|\le 2} \frac{ a_{\alpha,\beta}}{\alpha!\beta!} x^\beta \D^\alpha u(x) \d x
\end{equation}
für komplexe\footnote{Sind die $a_{\alpha,\beta}$ komplex, so existieren eindeutig bestimmte reelle Koeffizienten, welche dieselbe Form erzeugen.} Zahlen $a_{\alpha,\beta}\in\C$ und Funktionen $u\in\rmC_0^\infty(\R^n)$. Diese werden später als Taylorapproximanten auftreten und sollen zuerst untersucht werden.
...
\end{equation}
auf $\R^n\times\R^n$ und die symmetrische Grammatrix $H$ mit $\cspro{X}{H}{X}=h(X)$, $X=(x,\xi)\in\R^n\times\R^n$. Es ist naheliegend die Dirichletform $P$ mit Hilfe der kanonischen symplektischen Struktur auf $\R^n\times\R^n$ zu untersuchen, da wir das folgende Lemma haben.
\begin{lem}
Es sei $p$ ein Polynom zweiten Grades auf $\R^n\times\R^n$ und $\chi\in\mathrm{SP}_{2n}(\R)$ ein Element der symplektischen Gruppe. Dann ist die Ungleichung
\begin{align*}
\Re \int \overline{v(x)}p(x,\D)v(x)\d x\ge 0
\end{align*}
genau dann für alle $v\in S(\R^n)$ erfüllt, wenn sie für das Polynom $p\circ \chi$ erfüllt ist.
\end{lem}
Die symplektische Struktur ist durch $\cspro{X}{I}{X}$ mit
\begin{align*}
I=\left(\begin{matrix}
0 & E_n\\
-E_n& 0 
\end{matrix}\right)
\end{align*}
gegeben. Dabei ist $E_n$ die $n\times n$ Einheitsmatrix. Für eine reelle symmetrische Matrix $M$ hat  die Matrix $IM$ ein symmetrisch um den Ursprung angeordnetes, rein imaginäres Spektrum. Die positive Spur von $H$ sei nun
\begin{equation}
   {\mathrm{Tr}}^+ H = \sum_j\lambda_j 
\end{equation}
wobei die Folge $(\lambda_j)$ eine ihrer Vielfachheit entsprechenden Aufzählung der positiven Elemente von $i\sigma(IM)$ ist. Wir stellen fest, dass falls wir die quadratische Form $h$ symplektisch transformieren, die positive Spur der Grammatrix erhalten bleibt. Also für alle $\chi\in\mathrm{Sp}_{2n}(\R)$ gilt
\begin{align*}
...
Es gilt $P[u]\ge0$ genau dann für alle $u\in\rmC_0^\infty(\R^n)$, wenn die Bedingungen
\begin{enumerate}
...
\begin{equation}
    \Re \spro{Au}{u} + \epsilon \|u\|^2_{(\mu)}  \ge C\|u\|_{(s)}^2
\end{equation}
...
Da die Hessematrix $H_{a_m}$ homogen vom Grad $m-2$ ist und 
