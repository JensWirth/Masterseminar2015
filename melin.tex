...
Symplektische Transformation auf Normalform.
\end{proof}
...
%Da für das Symbol von $A_\rho$ die lokale  Entwicklung
%\begin{align*}
...
Nun sei 
\begin{align*}
 v\in\rmC^\infty_0(\R^n) \quad \text{und} \quad v_\lambda(x):=\lambda^{n/2}v(\lambda x)\, \e^{\i\lambda^2x\cdot \xi_0}\, ,\quad\lambda>0.
\end{align*}
...
\begin{align*}
&(\partial_\xi^\alpha\partial_x^\beta{a_k})(\widetilde x, \lambda^2 \widetilde \xi)(x/\lambda)^\beta(\lambda\xi)^\alpha = \mathcal O(1)\braket{\widetilde\xi}^{k-\abs\alpha}\braket{\xi}^{\abs\alpha}\lambda^{2k-\abs{\beta+\alpha}} = \mathcal O(1)\braket{\xi}^{k}\lambda^{2k-\abs{\beta+\alpha}}
\end{align*}
...
Es sei $P$ ein Pseudodifferentialoperator auf dem Gebiet $\Omega$ mit Symbol $\sigma$. Wir untersuchen, wie gut die zur Taylorentwicklung des Symbols zugehörigen Differentialoperatoren
\begin{align}
