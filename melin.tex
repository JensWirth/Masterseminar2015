...
\end{equation}
für komplexe\footnote{Sind die $a_{\alpha,\beta}$ komplex, so existieren eindeutig bestimmte reelle Koeffizienten, welche dieselbe Form erzeugen.} Zahlen $a_{\alpha,\beta}\in\C$ und Funktionen $u\in\rmC_0^\infty(\R^n)$. Diese werden später als Taylorapproximanten auftreten und sollen zuerst untersucht werden. Zugeordnet zu $P$ betrachten wir die (reelle) quadratische Form
\begin{equation}
  h(x,\xi) = \Re  \sum_{|\alpha+\beta|\le 2} \frac{a_{\alpha,\beta}}{\alpha!\beta!} x^\beta\xi^\alpha
\end{equation}
auf $\R^n\times\R^n$ und die symmetrische Matrix $H$ mit $\cspro{X}{H}{X}=h(X)$, $X=(x,\xi)\in\R^n\times\R^n$. Bezeichne weiter
\begin{equation}
   \widetilde{\mathrm{Tr}} H = ... 
\end{equation}
...
\item $f(x,\xi) = \sum_{|\alpha+\beta|=1} \Re a_{\alpha,\beta} x^\beta\xi^\alpha = 0$ für $(x,\xi)\in\R^{2n}$ mit $h(x,\xi)=0$;
\item $\Re a_{0,0} - \frac12 \sum_{|\beta|=1} \Im a_{\beta,\beta} -\frac14  \cspro{f}{H^{-1}}{f} + \widetilde{\mathrm{Tr}} H \ge 0$
\end{enumerate}
erfüllt sind.
\end{thm}
