...
\end{equation} 
Die Mengen $M_N$ sind abgeschlossen (siehe nachfolgendes Lemma \ref{lm:closed}),  konvex und symmetrisch. Weiter ist $F$ metrisierbar, der Bairesche Kategoriensatz impliziert also, dass mindestens eine der Mengen $M_N$ einen inneren Punkt (wegen Symmetrie den Ursprung) besitzt. Es gibt also ein $k$ und $\epsilon>0$, so dass
...
\end{equation}
Speziell mit $f=c f_{k,\tau}$ und $v=v_{k,N,\tau}$ folgt für $\tau\to\infty$ ein Widerspruch: Für $c>0$ klein genug ist dann $f\in M_N$. Weiter ist die rechte Seite wegen \eqref{eq:cond1} und \eqref{eq:cond2} beschränkt, die linke strebt mit \eqref{eq:cond3} aber gegen Unendlich.
\end{proof}

Das folgende Lemma liefert die im Beweis verwendete Abgeschlossenheit der Menge $M_N$.

\begin{lem}\label{lm:closed}
Sei $M_N$ wie in (\ref{eq:M_N}). Dann ist $M_N$ abgeschlossen.
\end{lem}

\begin{proof}
Die Menge der Distributionen, welche die erste Bedingung von (\ref{eq:M_N}) erfüllen, ist  bezüglich der schwachen Topologie auf $\mathscr{D}'(\omega_1)$ (folgen-)kompakt: Wir zeigen dies mit einem Diagonalfolgenargument. Zu zeigen ist also, dass
\begin{equation}
\{u\in\mathscr D'(\omega)\mid \forall_{\phi\in \rmC_0^\infty(\omega)}\quad |\langle u,\phi\rangle|\leq N \|\phi\|_{(N)}\}\subset\mathscr D'(\omega)
\end{equation}
kompakt ist.
Aufgrund der Darstellung
\begin{equation}
\rmC_0^\infty(\omega)=\bigcup\limits_{\omega_1\Subset\omega}\bigcap\limits_N H_0^N(\omega_1)
\end{equation}
und der Separabilität der Sobolevräume $H_0^N(\omega_1)$, finden wir eine abzählbare, dichte Teilmenge $\Phi=\{\phi_1,\phi_2,\ldots\}$ in $\rmC_0^\infty(\omega)$.
...
Sei also $(f_j)_{j\in\mathbb{N}}$ eine gegen $f\in B_0^\infty(\Omega)$ stark konvergente Folge in $M_N$, d.h. $f_j=Pu_j$ auf $\omega$ für gewisse $u_j\in\mathscr{D}'(\omega)$, welche die erste Bedingung aus (\ref{eq:M_N}) erfüllen. Dann existiert ein schwacher Grenzwert $u=\lim_{j\rightarrow\infty}u_j$, welcher aus Gründen der Separabilität von $H_0^N(\omega)$ ebenfalls (\ref{eq:M_N}) erfüllt. Dann ist aber wegen $Pu_j\rightarrow Pu$ in $\mathscr D'(\omega)$ auch $f=Pu$ in $\omega$ und somit auch $f\in M_N$, was die Abgeschlossenheit liefert.
\end{proof}
...
\end{equation}
ist abgeschlossen (vergleiche vorigies Lemma (\ref{lm:closed})), konvex und symmetrisch. \\\\
\textit{Schritt 2}\\
